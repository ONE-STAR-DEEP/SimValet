...
export const io = new Server(4001, {
  cors: {
    origin: "https://simvaletpark.thavertech.com",
    methods: ["GET", "POST"],
    credentials: true
  },
...
io.on("connection", (socket) => {
  ...
  socket.on("payment-update", (data) => {
    const valetId = data?.valetId;

    if (!valetId) {
      console.log("No valetId provided");
      return;
    }

    console.log(`Payment update for valet: ${valetId}`);

    io.to(`valet-${valetId}`).emit("payment-update", {
      success: true,
      valetId,
  ...
  socket.on("join-valet", (valetId) => {
    if (!valetId) return;

    socket.join(`valet-${valetId}`);
    socket.data.valetId = valetId;

    console.log(`Socket ${socket.id} joined valet ${valetId}`);
  });
